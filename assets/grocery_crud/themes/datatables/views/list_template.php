...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#datepicker').change(function() {
			$('.search_OD_DateTimeAdd').val($(this).val());
			$('.search_OD_DateTimeAdd').trigger('keyup');
			$('.search_OT_DateTimeAdd').val($(this).val());
			$('.search_OT_DateTimeAdd').trigger('keyup');
		});
		var uri_seg = window.location.pathname.split('/');
	    if (uri_seg[3] === 'control_cart') {
	    	if (uri_seg[4] === 'order_management') {
	    		if ($('table.groceryCrudTable th:first').attr('aria-sort') == 'ascending')
	    			$('table.groceryCrudTable th:first').click();
	    	}
	    	else if (uri_seg[4] === 'order_transfer_manage') {
	    		if ($('table.groceryCrudTable th:eq(1)').attr('aria-sort') == 'ascending')
	    			$('table.groceryCrudTable th:eq(1)').click();
	    	}
	    }
	});
</script>