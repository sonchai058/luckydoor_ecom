...
$(document).ready(function() {
	...
	if(!unset_export)
	{
		aButtons.push(    {
	         "sExtends":    "xls",
	         "sButtonText": '<i class="fa fa-file-excel-o"></i> ' + export_text,
	         "mColumns": mColumns
	     });
	}
	...
	if(!unset_print)
	{
		aButtons.push({
	         "sExtends":    "print",
	         "sButtonText": '<i class="fa fa-print"></i> ' + print_text,
	         "mColumns": mColumns
	     });
	}
...
function loadDataTable(this_datatables) {
	return $(this_datatables).dataTable({
	    ...
	    "fnFooterCallback": function(nRow, aaData, iStart, iEnd, aiDisplay) {
	    	var uri_seg = window.location.pathname.split('/');
	    	if (uri_seg[4] === 'product_management') {
				var iPageDebit = 0;
				var iPagePromo = 0;
				var iPageleave = 0;
				for (var i = iStart; i < iEnd; i++) {
					iPageDebit += parseFloat(aaData[aiDisplay[i]][3]) 	* 1;
					iPagePromo += parseFloat(aaData[aiDisplay[i]][4]) 	* 1;
					iPageleave += parseInt(aaData[aiDisplay[i]][5]) 	* 1;
				}
				var nCells = nRow.getElementsByTagName('th');
				nCells[2].innerHTML = 'ทั้งหมด';
				nCells[3].innerHTML = parseFloat(iPageDebit).toFixed(2);
				nCells[4].innerHTML = parseFloat(iPagePromo).toFixed(2);
				nCells[5].innerHTML = parseInt(iPageleave);
			}
			else if (
				uri_seg[4] === 'order_history_success' 	||
				uri_seg[4] === 'order_history_cancel'	||
				uri_seg[4] === 'order_doing_unannounce'	||
				uri_seg[4] === 'order_doing_announced'	 ) {
				var iPageDebit = 0;
				var iPagePromo = 0;
				for (var i = iStart; i < iEnd; i++) {
					bPageDebit  = aaData[aiDisplay[i]][3].replace(',', '');
					bPagePromo  = aaData[aiDisplay[i]][4].replace(',', '');
					iPageDebit += parseFloat(bPageDebit) 	* 1;
					iPagePromo += parseFloat(bPagePromo) 	* 1;
				}
				var nCells = nRow.getElementsByTagName('th');
				nCells[2].innerHTML = 'ทั้งหมด';
				nCells[3].innerHTML = parseFloat(iPageDebit).toFixed(2);
